...
def display_info():
    ...
    """Displays robot information on the controller screen."""
    # Get the absolute angle from the inertial sensor
    heading = inertial_sensor.heading(DEGREES)

    # Clear the first line of the controller screen
    controller.screen.clear_line(1)
    # Print the angle, formatted to two decimal places
    controller.screen.print("Angle: {:.2f}".format(heading))
