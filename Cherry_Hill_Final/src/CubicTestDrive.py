...
DEADBAND = 5
MAX_TURN_SCALE = 0.7     # limits turn aggressiveness
SLEW_RATE = 4            # max % change per loop
...
def cubic_scale(value):
    return (value ** 3) / 10000  # keeps max at 100

def slew_limit(target, current, rate):
...
def driving():
    ...
    # Cubic scaling
    forward = cubic_scale(forward)
    turn = cubic_scale(turn) * MAX_TURN_SCALE
    ...
    # Slew limiting
    current_left_speed = slew_limit(target_left, current_left_speed, SLEW_RATE)
    current_right_speed = slew_limit(target_right, current_right_speed, SLEW_RATE)
