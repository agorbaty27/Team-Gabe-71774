...
def driving():
    ...
    if abs(current_left_speed) < 5:
        current_left_speed = 0
    if abs(current_right_speed) < 5:
        current_right_speed = 0

#can delete this if you want, just a way to make the controller less twitchy at low speeds. feel free to mess with the exponent or threshold value
    if current_left_speed < 20:
        adjusted_left = current_left_speed**.8
    else:   
        adjusted_left = current_left_speed
    
    if current_right_speed < 20:
        adjusted_right = current_right_speed**.8
    else:
        adjusted_right = current_right_speed

# this is the original code before the smoothing adjustment, feel free to switch back if you want
#    left_drive.spin(FORWARD, (current_left_speed), PERCENT)
#    right_drive.spin(FORWARD, (current_right_speed), PERCENT)

# this is the code with the smoothing adjustment, feel free to adjust the exponent or threshold value to your liking
    left_drive.spin(FORWARD, (adjusted_left), PERCENT)
    right_drive.spin(FORWARD, (adjusted_right), PERCENT)
...
def matchload_score():
    ...
    wait(1.0, SECONDS)
    oscillate_drive(power = 30, time_ms=150, cycles=3)
...
def matchload_only():
    ...
    wait(0.25, SECONDS)
    intake.spin(REVERSE, 100, PERCENT)
    left_drive.spin(FORWARD, 8, PERCENT)
    right_drive.spin(FORWARD , 8, PERCENT)
    wait(6, SECONDS)
    intake.stop(COAST)
